...
import { AppHeader, ErrorState, LoadingState, PillButton, ScreenContainer, ScreenContent, ScreenScroll, SurfaceCard } from '@/components';
import { api } from '../../api/client';

const Caption = styled.Text({ color: '#66706B', marginBottom: 12 });
const BackButton = styled.Pressable({ alignSelf: 'flex-start', marginBottom: 12, padding: 8 });
const BackLabel = styled.Text({ color: '#66706B' });
...
export default function BookDetailsScreen() {
  ...
  const saveDetails = () => { setNotice(null); void run(async () => { const updated = await api.updateBook(book.id, { title, author, totalPages: Number(totalPages) }); setNotice('Изменения сохранены'); return updated; }); };
  const goBack = () => { if (router.canGoBack()) router.back(); else router.replace('/(tabs)/library'); };
  ...
  return <ScreenContainer><ScreenScroll><ScreenContent><BackButton accessibilityRole="button" onPress={goBack}><BackLabel>Назад</BackLabel></BackButton><AppHeader title={book.work.title} subtitle={book.work.authors.map((item) => item.name).join(', ')} /><SurfaceCard><Caption>{book.progress.currentPage} / {book.progress.totalPages} страниц · {labels[book.status]}</Caption><StatusRow>{statuses.map((status) => <PillButton key={status} label={labels[status]} variant={status === book.status ? 'primary' : 'secondary'} disabled={busy} onPress={() => void run(() => api.updateStatus(book.id, status))} />)}</StatusRow><Caption>Быстрый прогресс</Caption><StatusRow>{[5, 10, 25].map((delta) => <PillButton key={delta} label={`+${delta}`} disabled={busy} onPress={() => adjustPage(delta)} />)}</StatusRow><Caption>Текущая страница</Caption><Field value={page} onChangeText={setPage} placeholder={String(book.progress.currentPage)} keyboardType="number-pad" /><PillButton label="Сохранить прогресс" disabled={busy} onPress={updatePage} /></SurfaceCard><SurfaceCard><Caption>Данные книги</Caption><Field value={title} onChangeText={setTitle} placeholder="Название" /><Field value={author} onChangeText={setAuthor} placeholder="Автор" /><Field value={totalPages} onChangeText={setTotalPages} placeholder="Страницы" keyboardType="number-pad" /><PillButton label={busy ? 'Сохраняем…' : 'Сохранить книгу'} disabled={busy || !title.trim() || !author.trim() || Number(totalPages) < 1} onPress={saveDetails} /><PillButton label="Удалить книгу" variant="secondary" disabled={busy} onPress={remove} /></SurfaceCard>{notice ? <Caption>{notice}</Caption> : null}{error ? <ErrorState message={error} /> : null}</ScreenContent></ScreenScroll></ScreenContainer>;
}
